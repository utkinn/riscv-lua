...
local opcode, funct3, funct7 = getOpcodeAndFuncsForMnemonic("sb")
...
describe("sb", function()
    ...
    describe("stores a byte in rs2 to address rs1+imm", function()
        it("(no offset)", function()
            ...
            assert.are.equal(0x34, memory:readByte(0x0))
        end)
        ...
        it("(offset = 1)", function()
            ...
            assert.are.equal(0x34, memory:readByte(0x1))
        end)
