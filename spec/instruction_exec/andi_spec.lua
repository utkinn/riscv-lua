...
local opcode, funct3, funct7 = getOpcodeAndFuncsForMnemonic("andi")
...
describe("andi", function()
    ...
    it("regular", function()
        cpu.registers[1] = tonumber("11", 2)
        instructionData.rd = 1
        instructionData.rs1 = 1
        instructionData.imm = tonumber("111", 2)
        local inst = Instruction.new(instructionData)

        inst:exec(cpu)

        assert.are.equal(tonumber("11", 2), cpu.registers[1])
    end)
