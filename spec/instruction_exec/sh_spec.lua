...
local opcode, funct3, funct7 = getOpcodeAndFuncsForMnemonic("sh")
...
describe("sh", function()
    ...
    describe("stores a half-word in rs2 to address rs1+imm", function()
        it("(no offset)", function()
            ...
            assert.are.equal(0x5678, memory:readHalfWord(0x0))
        end)
        ...
        it("(offset = 1)", function()
            ...
            assert.are.equal(0x5678, memory:readHalfWord(0x1))
        end)
